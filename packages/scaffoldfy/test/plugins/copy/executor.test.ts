...
describe('executeCopy', () => {
  ...
  it('should copy a file to a new location', async () => {
    ...
    expect(fs.readFileSync('source.txt', 'utf-8')).toBe('test content');
    expect(fs.readFileSync('destination.txt', 'utf-8')).toBe('test content');
  });

  it('should copy a file to a nested directory', async () => {
    // Create source file
  ...
  it('should copy a directory recursively', async () => {
    ...
    fs.mkdirSync('source-dir/sub-dir', { recursive: true });
    fs.writeFileSync('source-dir/file1.txt', 'content1');
    fs.writeFileSync('source-dir/file2.txt', 'content2');
    fs.writeFileSync('source-dir/sub-dir/file3.txt', 'content3');
    ...
    expect(fs.existsSync('source-dir')).toBe(true);
    expect(fs.existsSync('source-dir/file1.txt')).toBe(true);

    // Destination directory exists with all files
    ...
    expect(fs.existsSync('destination-dir/sub-dir/file3.txt')).toBe(true);
    ...
    expect(fs.readFileSync('destination-dir/sub-dir/file3.txt', 'utf-8')).toBe(
      'content3',
    );
  });

  it('should skip copy if source does not exist', async () => {
    const config: CopyConfig = {
  ...
  it('should skip copy when condition is false', async () => {
    // Create source file
    fs.writeFileSync('source.txt', 'test content');
  ...
  it('should execute copy when condition is true', async () => {
    // Create source file
    fs.writeFileSync('source.txt', 'test content');
